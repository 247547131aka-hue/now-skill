...
```bash
cd ~/.claude/skills/now-skill && git pull
```
...
不支持 Skill 机制，但可以直接把仓库里 SKILL.md 的内容复制到对话中，手动跟随指引操作。

**方式 4：其他 Agent（Cursor / Cline / Codex 等）**

各平台的 Skill/Plugin 机制不同，可将 `SKILL.md` + `prompts/` 中的提示词作为参考改写适配。

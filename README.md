...
**方式 1：Claude Code 一键安装**

```bash
git clone https://github.com/247547131aka-hue/now-skill ~/.claude/skills/now-skill
```

**方式 2：其他平台（Cursor / Cline / Codex / Copilot Chat 等）**

1. 下载本仓库到本地
2. 将 `SKILL.md` + `prompts/` 作为提示词输入给对应 Agent
3. 各平台 Skill/Plugin 机制不同，以下为通用安装参考：

| 平台 | 安装方式 |
|------|---------|
| **Claude Code** | `/plugin install 247547131aka-hue/now-skill` 或 `git clone` 到 `~/.claude/skills/` |
| **Claude.ai 网页版** | 不支持 Skill 机制，将 SKILL.md 内容粘贴到对话中手动操作 |
| **Cursor** | 放到 `.cursor/skills/` 目录，或作为 `.cursorrules` 规则引用 |
| **Cline (VS Code)** | 在 `.clinerules` 中引用，或作为上下文文件加载 |
| **Codex (OpenAI)** | 将 prompts/ 下的指令改编为 Codex 的 agent 配置 |
| **Continue (VS Code)** | 放到 `~/.continue/config.json` 的自定义 prompt 中 |

> 💡 不管哪种平台，核心逻辑不变：信息采集 → 原材料导入 → 人格+记忆生成 → Skill 输出。
